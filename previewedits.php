<!-- 
previewedits.php
	- gets directed to from editaccept.php
	- shows a preview from the content stored in the edit request tables
	- edit request tables are populated by entryedit.php
-->

<?php 
	// pulls all the relevant information from the edit request tables
	require 'includes/header.php';
	include_once 'includes/dbhandler.php';
	$gid = $_GET['id'];
	$sql = "SELECT * FROM edit_requests WHERE request_id=$gid";
	...
	$sql2 = "SELECT * FROM picture_edit_requests WHERE pic_id=$gid;";
	$picQuery = mysqli_query($conn, $sql2);
	$row2 = mysqli_fetch_assoc($picQuery);

	$sql3 = "SELECT * FROM tag_edit_requests WHERE tag_id=$gid;";
	$tagQuery = mysqli_query($conn, $sql3);
	$row3 = mysqli_fetch_assoc($tagQuery);

?>

<!-- alert to redirect back to editaccept.php --> 
<div class="alert alert-warning navbar-fixed-bottom" role="alert" align="center">
  <a class="alert-link">Notice:</a> <em>You are currently viewing the preview of a game page that has not yet been added. Click <a class = "alert-link" href="editaccept.php">here</a> to return to the decision page.</em>
</div>

<!-- duplicate code from entry.php -->
